...
/**
 * Artigos de /insights. Moram aqui (e não na página) para a listagem, a rota
 * [slug] e o sitemap lerem a mesma fonte: data do post, tempo de leitura e
 * links relacionados não divergem mais entre as telas.
 * Quando migrar para MDX, esta é a interface a manter.
 */
...
export interface Post {
  /** Título do artigo (H1 e listagem). */
  title: string
  ...
  date: string
  /** Data da última revisão relevante do conteúdo, AAAA-MM-DD. */
  dateModified?: string
  /** Markdown renderizado por MarkdownContent. */
  content: string
  /** Bloco 'Veja também': serviço correspondente + outro artigo/case. */
  related: RelatedLink[]
}
...
export const POSTS: Record<string, Post> = {
  'reforma-tributaria-2026': {
    ...
    metaTitle: 'Reforma tributária 2026: IBS, CBS e seu sistema',
    description: 'Reforma tributária 2026: IBS e CBS mudam cálculo fiscal, emissão de nota e reporting. O que a empresa que emite nota fiscal precisa preparar primeiro.',
    category: 'Reforma tributária',
    date: '2026-06-30',
    related: [
      { href: '/desenvolvimento-de-sistemas', label: 'Desenvolvimento de sistemas sob medida com integração SEFAZ' },
      { href: '/cases/nf-saas', label: 'Case NF SaaS: plataforma fiscal preparada para IBS/CBS' },
      { href: '/insights/dctfweb-automacao', label: 'DCTFWeb sem erro: automação real para escritórios contábeis' },
    ],
    content: `
...
`,
  ...
  'migrar-visual-foxpro-web': {
    ...
    metaTitle: 'Migrar Visual FoxPro para web sem parar operação',
    description: 'Como migrar Visual FoxPro para web sem parar a operação: migração em paralelo, validação por checksum e corte cirúrgico no fim de semana.',
    ...
    related: [
      ...
      { href: '/cases/prossiga', label: 'Case Prossiga: Visual FoxPro para Supabase sem parar a receita' },
      { href: '/consultoria-ti', label: 'Consultoria em TI: plano de modernização de legado' },
    ],
    content: `
...
`,
  ...
  'whatsapp-business-api-empresarial': {
    ...
    metaTitle: 'WhatsApp Business API: arquitetura segura',
    description: 'WhatsApp Business API empresarial sem risco de banimento ou vazamento de dados: a arquitetura segura para escalar o atendimento com controle.',
    category: 'Automação',
    date: '2026-06-30',
    related: [
      { href: '/desenvolvimento-de-sistemas', label: 'Desenvolvimento de sistemas e integrações sob medida' },
      { href: '/seguranca-lgpd', label: 'Segurança da informação e adequação LGPD' },
    ],
    content: `
...
`,
  ...
  'dctfweb-automacao': {
    ...
    metaTitle: 'DCTFWeb sem erro: automação para contabilidades',
    description: 'DCTFWeb sem erro: por que a maioria das automações de escritório contábil é frágil e como construir uma que não falha no fim do mês.',
    category: 'Automação fiscal',
    date: '2026-05-23',
    related: [
      { href: '/desenvolvimento-de-sistemas', label: 'Desenvolvimento de sistemas sob medida com automação fiscal' },
      { href: '/raio-x-de-ti', label: 'Raio-X de TI para escritórios contábeis' },
      { href: '/insights/lgpd-escritorio-contabil', label: 'LGPD no escritório contábil: checklist de 23 itens' },
    ],
    content: `
...
`,
  ...
  'lgpd-escritorio-contabil': {
    ...
    metaTitle: 'LGPD no escritório contábil: 23 itens auditados',
    description: 'LGPD no escritório contábil: o checklist de 23 itens que aplicamos em auditorias reais de quem trata dados sensíveis em volume industrial.',
    category: 'LGPD',
    date: '2026-05-23',
    related: [
      { href: '/seguranca-lgpd', label: 'Segurança da informação e adequação LGPD' },
...
export function getPost(slug: string): Post | undefined {
  return Object.prototype.hasOwnProperty.call(POSTS, slug) ? POSTS[slug] : undefined
}

export function postReadingTime(post: Post): number {
  return readingTimeMinutes(post.content)
}

/** Data de referência para lastmod/dateModified. */
...
/**
 * '2026-06-30' → '30 de junho de 2026'. Parse manual de propósito:
 * new Date('AAAA-MM-DD') é UTC e, no fuso de Brasília, mostraria o dia anterior.
 */
