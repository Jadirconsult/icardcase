...
export function WhatsAppButton({
  ...
  const handleClick = useCallback(() => {
    ...
    fetch('/api/whatsapp-click', {
      ...
      body: JSON.stringify({ origem, utm_source, utm_campaign }),
      keepalive: true,
    }).catch(() => {})
  }, [origem])
  ...
  if (variant === 'float') {
    return (
      <a
        ...
        className={cn(
          'fixed bottom-6 right-6 z-50 flex h-14 w-14 items-center justify-center rounded-full bg-[#25D366] text-white shadow-lg shadow-black/20 transition-transform hover:scale-105 hover:shadow-xl',
          className,
        )}
      >
